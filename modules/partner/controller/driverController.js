...
class DriverController {
  addDriver = async (req, res) => {
    try {
      ...
      if (checkPhone) {
        throw new Error(
          "This phone number is already registered with the same partner. A partner cannot add the same phone number for a driver more than once."
        );
      }

      const driverImage = req.file
        ? await uploadToCloudinary(req, req.file.path, "driverImage")
        : null;

      const driver = await driverService.addDriver({
        partnerId,
        firstName,
        lastName,
        phoneNumber,
  ...
  getDriversByPartner = async (req, res) => {
    try {
      ...
      const drivers = await driverService.getDriversByPartnerId({
        partnerId,
        limit,
        page,
      });
  ...
  deleteDriver = async (req, res) => {
    try {
      ...
      const deletedDriver = await driverService.deleteDriver({
        driverId,
        partnerId,
      });
      ...
      if (!deletedDriver) {
        return res
          .status(404)
          .json({ message: "Driver not found or already deleted" });
      }
