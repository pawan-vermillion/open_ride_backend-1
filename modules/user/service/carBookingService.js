...
class CarBookingService {
    ...
    async searchCar({ pickUpDate, pickUpTime, pickupLocation, returnDate, returnTime, filters, pagination }) {
        try {
          const { price, model, company, carType, seat, door, modelYear, transmission, fuelType } = filters || {};
          ...
          if (model) filteredCars = filteredCars.filter(car => car.modelName.model === model);
          if (company) filteredCars = filteredCars.filter(car => car.companyName.carCompany === company);
          if (carType) filteredCars = filteredCars.filter(car => car.bodyStyle.bodyStyle === carType);
          if (seat) filteredCars = filteredCars.filter(car => car.numberOfSeat == seat);
          if (door) filteredCars = filteredCars.filter(car => car.numberOfDoors == door);
          if (modelYear) filteredCars = filteredCars.filter(car => car.modelYear === modelYear);
